 ...
 * ストレージに保存される値の種類。
 *
 * "number", "ordered-number" は数値 (Infinity と NaN を除く)。
 ...
 * "number" の最小値。
 */
export const STORAGE_MIN_NUMBER: number = -Number.MAX_VALUE;

/**
 * "number" の最大値。
 */
export const STORAGE_MAX_NUMBER: number = Number.MAX_VALUE;

export interface StorageData {
	/**
	 * 誰の値か。
	 * いずれかのインスタンスの g.game.selfId に与えられる文字列または空文字列 "" (プレイヤーに紐づけない場合) でなければならない。
	 * 空文字列 "" にした場合でも、プレイヤーに紐づかない playerId として扱われる。
	 */
	playerId: string;
