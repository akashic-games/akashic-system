...
(function(a) {
  ...
  b.ext.renderer.header.jqueryui = function(b, h, e, c) {
    var f = "css_right ui-icon ui-icon-carat-2-n-s",
      g = -1 !== a.inArray("asc", e.asSorting),
      i = -1 !== a.inArray("desc", e.asSorting);
    !e.bSortable || (!g && !i)
      ? (f = "")
      : g && !i
      ? (f = "css_right ui-icon ui-icon-carat-1-n")
      : !g && i && (f = "css_right ui-icon ui-icon-carat-1-s");
    a("<div/>")
    ...
    a(b.nTable).on("order.dt", function(a, g, i, j) {
      b === g &&
        ((a = e.idx),
        h.removeClass(c.sSortAsc + " " + c.sSortDesc).addClass("asc" == j[a] ? c.sSortAsc : "desc" == j[a] ? c.sSortDesc : e.sSortingClass),
        h
          .find("span." + c.sSortIcon)
          .removeClass(
            "css_right ui-icon ui-icon-triangle-1-n css_right ui-icon ui-icon-triangle-1-s css_right ui-icon ui-icon-carat-2-n-s css_right ui-icon ui-icon-carat-1-n css_right ui-icon ui-icon-carat-1-s"
          )
          .addClass(
            "asc" == j[a] ? "css_right ui-icon ui-icon-triangle-1-n" : "desc" == j[a] ? "css_right ui-icon ui-icon-triangle-1-s" : f
          ));
    });
  };
  b.TableTools &&
